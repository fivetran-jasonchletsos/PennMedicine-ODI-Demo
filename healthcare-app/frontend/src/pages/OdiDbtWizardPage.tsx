...
const PILLARS: Pillar[] = [
  ...
  },
  {
    ...
    accent: '#FF694A',
    tag: 'transform',
    what: 'Bronze, silver, gold transformations with declarative SQL, run natively on Databricks compute. Lineage, tests, freshness SLAs, semantic models. dbt-wizard adds four sub-agents that author new models into the project using the same tools an analytics engineer uses.',
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <section className="mb-12">
        <div className="eyebrow mb-2">Three layers. One loop.</div>
        ...
        <p className="text-sm text-[var(--ink-muted)] mb-6 max-w-3xl">
          ...
          answered in time. Pull Databricks out and the materialization step has no warehouse and no
          governing catalog. Pull Fivetran out and there is nothing new to build on. The loop only
          ...
          {PILLARS.map((p, i) => (
            <div key={p.vendor} className="clinical-card relative flex flex-col" style={{ minHeight: '420px', borderTop: `3px solid ${p.accent}` }}>
              <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="text-[11px] font-mono uppercase tracking-wider mb-1" style={{ color: 'var(--ink-soft)' }}>At Penn Medicine</div>
      ...
      <section className="mb-12" style={{ background: 'var(--paper-deep)', border: '1px solid var(--hairline)', borderRadius: '0.25rem', padding: '1.5rem' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { num: '03', name: 'Worker', tools: 'dbt warehouse, dbt_show, file edit', job: 'Writes the SQL and runs a dbt_show slice against a live serverless SQL warehouse. Validates the proposed daily service-line grain, checks row counts, and edits the model file into the project.' },
      ...
      <section className="rounded-sm p-8" style={{ background: 'var(--color-brand-900, #0b2744)', color: '#fff' }}>
        ...
        <p className="font-serif text-xl sm:text-2xl leading-snug mb-6">
          ...
          <span style={{ color: '#FF694A' }}>dbt Labs governs and builds it.</span>{' '}
          <span style={{ color: '#FF694A' }}>dbt-wizard authors it.</span>{' '}
          <span style={{ color: '#FF3621' }}>Databricks stores and serves it.</span>
        </p>
        <p className="text-sm opacity-70 max-w-2xl mb-6">
          Build-time AI on the same lakehouse as every downstream reader. dbt-wizard authors the model.
          Unity Catalog governs it and Databricks serves it — no integration handoff, no second copy of the data.
        </p>
